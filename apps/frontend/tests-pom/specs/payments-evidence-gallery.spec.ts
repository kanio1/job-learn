...
test('PW-OPS-E2E-220 upload 3 images → next → index 2', async ({ app, api, ownedMerchantId }, testInfo) => {
  ...
  for (let i = 0; i < 3; i++) {
    await app.page.getByTestId('evidence-upload-input').setInputFiles({
      name: `shot-${i}.png`,
      mimeType: 'image/png',
      buffer: PNG,
    })
    await app.page.getByTestId('evidence-upload-submit').click()
    await expect(app.page.getByTestId('evidence-file-name').filter({ hasText: `shot-${i}.png` })).toBeVisible()
  }
  await app.paymentDetail.evidenceCarousel.expectOpen()
  await app.paymentDetail.evidenceCarousel.goNext()
  await app.paymentDetail.evidenceCarousel.goNext()
  await app.paymentDetail.evidenceCarousel.expectIndex(2)
})
...
test('PW-OPS-E2E-221 ArrowRight keyboard', async ({ app, api, ownedMerchantId }, testInfo) => {
  ...
  await client.uploadEvidence(ownedMerchantId, paymentOrderId, { name: 'a.png', mimeType: 'image/png', buffer: PNG })
  await client.uploadEvidence(ownedMerchantId, paymentOrderId, { name: 'b.png', mimeType: 'image/png', buffer: PNG })
  await app.paymentDetail.gotoOrder(ownedMerchantId, paymentOrderId)
  await app.paymentDetail.expectLoaded()
  await app.paymentDetail.evidenceCarousel.expectOpen()
  await app.paymentDetail.evidenceCarousel.region().focus()
  await app.page.keyboard.press('ArrowRight')
  await app.paymentDetail.evidenceCarousel.expectIndex(1)
})
...
test('PW-OPS-E2E-223 invalid id error slide', async ({ app, api, ownedMerchantId }, testInfo) => {
  ...
  await client.uploadEvidence(ownedMerchantId, paymentOrderId, { name: 'ok.png', mimeType: 'image/png', buffer: PNG })
...
test('PW-OPS-E2E-224 other merchant evidence 404', async ({ app, api, ownedMerchantId, playwright }, testInfo) => {
  const client = requireApi(api)
  ...
  const otherApi = await BffClient.create(playwright, workerManagerAuthFile(other.index))
  try {
    const otherOrder = await createOrder(otherApi, other.merchantId, testInfo, 'BOLA2')
    const foreign = await otherApi.uploadEvidence(
      other.merchantId,
      otherOrder,
...
test('PW-OPS-API-070 GET evidence unknown id 404 problem', async ({ api, ownedMerchantId }, testInfo) => {
  ...
  const missing = randomUUID()
  const response = await client.getEvidence(ownedMerchantId, paymentOrderId, missing)
  expect(response.status).toBe(404)
  const contentType = String(response.headers['content-type'] ?? '')
  expect(contentType).toContain('application/problem+json')
  const raw = response.raw.toString('utf8')
