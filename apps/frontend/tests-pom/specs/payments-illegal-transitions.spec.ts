...
async function etag(client: BffClient, merchantId: string, paymentOrderId: string) {
  const get = await client.payments.get(merchantId, paymentOrderId)
...
async function bringTo(
  ...
  if (target === 'CANCELLED') {
    const cancelled = await client.payments.cancel(
...
test('illegal lifecycle edges stay on BFF REST (SCN-ILL)', async ({ api, ownedMerchantId }, testInfo) => {
  ...
  for (const row of illegalStDtRest) {
    ...
    const result = row.action === 'capture'
      ? await client.payments.capture(ownedMerchantId, paymentOrderId, match, key, 2100)
      : row.action === 'refund'
        ? await client.payments.refund(ownedMerchantId, paymentOrderId, match, key)
        : row.action === 'cancel'
          ? await client.payments.cancel(ownedMerchantId, paymentOrderId, match, key)
          : await client.payments.authorize(ownedMerchantId, paymentOrderId, match, key)
...
test('If-Match BVA on authorize is REST (absent 428, v99 412, malformed 400)', async ({ api, ownedMerchantId }, testInfo) => {
  ...
  for (const row of authorizeHeaderBoundaries.filter(item => item.ifMatch !== 'fresh')) {
    ...
    const result = await client.payments.authorize(
...
test('If-Match × action DT stays on BFF REST (SCN-IFM)', async ({ api, ownedMerchantId }, testInfo) => {
  ...
  for (const row of ifMatchActionMatrix) {
    ...
    const key = uniqueIdempotencyKey(testInfo, `${row.id}-ACT`)
    const result = row.action === 'capture'
      ? await client.payments.capture(ownedMerchantId, paymentOrderId, ifMatch, key, 2100)
      : await client.payments.cancel(ownedMerchantId, paymentOrderId, ifMatch, key)
    expectStatus(result, row.expectStatus, row.id)
    expectProblem(result.body, row.expectStatus)
    const after = await client.payments.get(ownedMerchantId, paymentOrderId)
    expect(after.body?.status, row.id).toBe(row.to)
