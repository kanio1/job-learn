...
- **Kafka Event Streaming Lab (2026-08-21):** **DESIGNED_NOT_STARTED — czeka na akceptację P0 (ADR 0002) przez użytkownika.** Review task force propozycji: [.codex/research/kafka-event-streaming-proposal-review.md](../.codex/research/kafka-event-streaming-proposal-review.md) (verdict APPROVE_WITH_CHANGES; korekty: Flyway max **V36** → eventlab rezerwuje **V37+**, Awaitility nieobecna w pom, externalizacja programistyczna w `eventlab` zamiast `@Externalized` w shared, stabilny `eventId` + reuse ops frames, temat 3 partycje + nagłówki v1). ADR [0002 PROPOSED](../.codex/adr/0002-kafka-event-streaming.md), spec [.codex/specs/kafka-event-streaming-lab.md](../.codex/specs/kafka-event-streaming-lab.md), backlog wykonawczy [status/roadmaps/kafka-event-streaming-lab/](roadmaps/kafka-event-streaming-lab/) (E0–E5, taski `KAFKA-T00`…`T18`), katalog testów [docs/testing/event-streaming-lab/](../docs/testing/event-streaming-lab/) (`BC-/UC-KAFKA-*`, `AT-`, `RA-KAFKA-*`, `PW-KAFKA-E2E/API/SEC-*`). Decyzja narzędzie Kafka: **Java Testcontainers primary oracle** (SUT JVM); TS = opcjonalny read-only probe `tools/kafka-probe` (kafkajs, poza apps/frontend); Playwright live dla UI/BFF bez `page.route`/`routeWebSocket`. Zero kodu aplikacji. `.kiro/**` unchanged.
