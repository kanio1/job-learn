...
## When Not to Use
Do not use this for Playwright framework mechanics (`playwright-pom`), backend behavior, OAuth/OIDC integration, or replacing Nuxt UI with a custom visual system. Product Vue/Nitro placement is `.agents/skills/nuxt-frontend`. This skill is dashboard taste and testability.
...
## Spec integration
- Architectural change (new page, composable layer, major restructure): `to-spec` / `to-tickets` under `.codex/`.
- Prefer small, reviewable changes. Ask before large implementation.
- Use `.kiro/steering/frontend-nuxt-ui.md` as persistent context.

## Boundaries With Other Skills
- Product placement: `nuxt-frontend`.
- Playwright POM / fixtures: `playwright-pom`.
- Backend / security / REST matrix: `spring-modulith`, `rest-api-test-design`.

See [`references/frontend-testability-checklist.md`](references/frontend-testability-checklist.md).
