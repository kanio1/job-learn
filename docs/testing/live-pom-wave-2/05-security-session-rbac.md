...
### PW-W2-SEC-005 — Cookie &lt; 4 KB; brak `id_token` w sesji

| | |
|---|---|
| Pokrycie | existing-pom `expectSessionCookieUnderUaLimit` · UC-SESS-04 |
| Prio | P0 |
| Uczy | Drugi JWT w `nuxt-session` zrzuca ciasteczko (UA ~4096 B). Produkt: tylko `accessToken`. |

### PW-W2-SEC-006 — SameSite + Secure z `context.cookies()`

| | |
|---|---|
| Pokrycie | existing-pom `expectSessionCookieSameSiteLax` (HttpOnly = SEC-001). TLS Secure = RFC E2E-056 existing |
| Prio | P1 |
...
| Prio | P0 |
| Uwaga | ponowny `/admin/merchants` po Unlock: existing-pom MRL E2E-022 |
...
| | |
|---|---|
| Pokrycie | existing-pom E2E-121 (fail + happy + wrong token); kontrast merchant POST existing-pom `admin-bff.spec.ts` |
| Prio | P1 |
