...
| Bank-like | step-up header, statements, disputes, maker-checker, AIS-lite | RA + POM rbac + mocked PDF | żywy PDF magic przez BFF; TPP 429; evidence UI 413 |
...
| ID | Produkt | Test |
|---|---|---|
| FR-V01–V05 | Visual Lab tiles + break tag | existing-pw screenshots |
| FR-V04 full+mask | `visual-lab-mask.css` na kafelkach | Partial — brak full-page |
| FR-N01 | Nitro 503→200, TTL 10s | existing-pw fulfill + existing-pom live |
| FR-N02 abort | `network-lab/slow` | existing-pw abort; designed offline |
| FR-N03 lie | `lie-fulfillment` | existing-pw body success; designed fulfillment oracle |
| FR-N04 strip | — | designed |
| FR-N05 HAR | fixture `network-lab.har` | existing-pw replay |
| FR-N06 CORS | `/api/network-lab/cors-cookie` | designed OPTIONS |
...
| FR-P05 iframe | same-origin widget | existing-pw frameLocator Approve |
...
| FR-B02 | CSV/PDF + BFF binary | existing-pw mocked PDF `%PDF-`; POM CSV |
...
|---|---|---|---|
| GAP-T01 | FE flag off: nav 0 + BFF 404 bez backendu | PW project env | P0 |
| GAP-T02 | Idle TTL-1 vs TTL+1; overlay na **każdym** `/admin/**` | POM clock | P0 |
| GAP-T03 | CSRF happy path + merchant POST bez CSRF nadal 2xx | PW request | P1 |
| GAP-T04 | PDF magic bytes przez **żywy** BFF (nie fulfill) | POM download | P1 |
| GAP-T05 | TPP 31. GET → 429; header vs query precedence | RA | P1 |
| GAP-T06 | Evidence UI 413/415; filename `../` | POM + RA | P1 |
| GAP-T07 | CORS OPTIONS credentials | PW request | P1 |
| GAP-T08 | 503 counter reset po TTL 10s (izolacja) | Nitro/PW | P1 |
